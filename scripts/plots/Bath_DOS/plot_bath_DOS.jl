...
fig = Figure(figure_padding=(1, 2, 1, 4), fonts=(;regular=projectdir("fonts", "MinionPro-Capt.otf")), size=(HokseonPlots.RESOLUTION[1]*2, HokseonPlots.RESOLUTION[2]*3))
ax1 = Axis(fig[1,1]; limits=(-27, 27, -1, 45), xgridvisible=false,ygridvisible=false, xticksmirrored = false,xticksvisible = false)
ax2 = Axis(fig[2,1]; xlabel="Energy / eV", limits=(-27, 27, -1, 45), xgridvisible=false,ygridvisible=false,xticksvisible = true, xticksmirrored = false, xlabelsize=20)
axes = [ax1, ax2]
...
Label(fig[:,0], "Density / eV⁻¹", rotation=π/2, padding=(0, -10, 0, 0),fontsize=20)
