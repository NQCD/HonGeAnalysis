...
function plot_fig_5(; Scattered_or_not::Bool=true, is_exp_plot::Bool=false)
    ...
    for (i,incident_energy) in enumerate(incident_energies)
        ...
        energy_loss = begin data = readdlm(projectdir("figure_data","fig_5","IESH_energyloss_$(incident_energy)_eV.txt"), ' ', skipstart=1); data[:, 1] end
