...
function plot_final_sigma_multi_csv(params_list; saving = false)
    # Create your figure with Minion Pro as the default font
    fig = Figure(
        size = (HokseonPlots.RESOLUTION[1] * 3.2, 5.5 * HokseonPlots.RESOLUTION[2]),
    ...
    axes = [
        Axis(
            ...
            limits = (3.9, 7.6, -0.25, ymax[i]), # Y limits set automatically or define below
    ...
    HEOM_src = datadir("sims", "HEOM", "tighterTolerance","KE_distributions_sigma_data_617eV.txt")
    ...
    for (i,param) in enumerate(params_list)
        ...
        lines!(axes[i], HEOM_x, HEOM_y ./ area, color=:blue, linewidth=3, label="HEOM Final Scattering")
        ...
        Label(fig[i,1], "σ = $(param["sigma"])"; tellwidth=false, tellheight=false, valign=:top, halign=:right, padding=(10,10,10,10),fontsize=25)
...
fig = plot_final_sigma_multi_csv(params_list; saving = false)
#save(plotsdir("HEOM_IESH_617_multi_sigmas_tighterTolerance.pdf"), fig)
display(fig)
