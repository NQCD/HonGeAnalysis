"""
plot_energy_loss_hist_csv_wigner_compare.jl
...
function plot_exp_inelastic_data!(ax, incident_energy)
    ...
    try
        H_Ge_data_x = H_Ge_data[:, 1]
        H_Ge_data_y = H_Ge_data[:, 2]

        interp = LinearInterpolation(H_Ge_data_x, H_Ge_data_y)

        integration = quadgk(interp, minimum(H_Ge_data_x), maximum(H_Ge_data_x))[1]
    catch e
        @info "Imported data does not contain inelastic peak"
    end

    # Plot the experimental data
    scatter!(ax, H_Ge_data[:, 1], H_Ge_data[:, 2] ./ integration, color=:white, marker=:circle, markersize=15, label = "H/Ge Experiment", strokewidth = 2)
...
function plot_energy_loss_hist_csv(params_list::Vector{Dict{String, Any}}; Scattered_or_not::Bool=true, is_exp_plot::Bool=false)
    ...
    ax = MyAxis(
        ...
        limits = (-0.1, nothing, -0.1, 2),
        xgridvisible = false,
        ygridvisible = false
    )
    ## Plotting the experimental data and area under the inelastic peak
    intergation = is_exp_plot ? plot_exp_inelastic_data!(ax, incident_energy) : 1

    color_vec = [:blue, :green, :red, :orange, :purple]
    ...
    for (i,param) in enumerate(params_list)
        ...
        @info "Total trajectories of incident energy $incident_energy eV: $(length(energy_loss)) with is_Wigner = $is_Wigner"


        peak_type_vec = ["inelastic", "elastc"]

        n_loss = length(filter(x -> x > 0.1, energy_loss))
        @info "number of non-zero losses events $n_loss"

        for peak_type in peak_type_vec
            if peak_type == "inelastic"
                energy_loss_plot = filter(x -> x > 0.1, energy_loss)
                label_add = true
                std = 0.02
            else
                energy_loss_plot = filter(x -> x < 0.1, energy_loss)
            ...
            k = kde(energy_loss_plot, Normal(0, std))

            if is_Wigner
                IESH_label = "IESH (Wigner)"
            else
                IESH_label = "IESH ($(incident_energy) eV, 5 Å)"
            end
            ...
            lines!(ax, k.x, k.density , color=color_vec[i], linewidth=3, label=label_add ? IESH_label : nothing)
...
is_exp_plot = true
saving = false  # save the figure value into the txt file in figure_data/fig_5 folder or not
@unpack incident_energy = params_list[1]
