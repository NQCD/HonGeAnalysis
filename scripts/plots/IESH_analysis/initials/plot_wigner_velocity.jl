...
function plot_wigner_velocity(v_distribution)
    fig = Figure(
    ...
    ax = MyAxis(
        fig[1, 1],
        xlabel = "Velocity",  # Wrap text parts in \text{}
        ylabel = "Probability Density",
        limits = (-0.007, 0.0, -0.1, nothing),
        xgridvisible = false,
        ygridvisible = false
    )
    ...
    μ = v_distribution.μ
    σ = v_distribution.σ
    v_range_au = collect(range(μ-5σ, μ+5σ, length=1000))
    ...
    lines!(ax, v_range_au, f_v_au, color=colormap[3], linewidth=2, label="Wigner Distribution")
...
plot_wigner_velocity(v_distribution)