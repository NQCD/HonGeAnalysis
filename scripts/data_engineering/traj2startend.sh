...
echo "Starting Julia script traj2startend.jl..."
# Execute nohup first to get its PID
nohup julia traj2startend.jl > /dev/null 2>&1 & # Temporarily redirect to /dev/null
NOHUP_PID=$! # Capture the PID of the nohup process

# Define the log file name using the machine name and the captured nohup PID
LOG_FILE="${LOG_DIR}/traj2startend_${HOSTNAME}_${NOHUP_PID}.log"
...
LOG_FILE="${LOG_DIR}/traj2startend_${HOSTNAME}_${SCRIPT_PID}.log"

echo "Restarting Julia script traj2startend.jl with log file: $LOG_FILE"
nohup julia traj2startend.jl > "$LOG_FILE" 2>&1 &
