...
nohup julia kineticlossstats.jl > logs/kineticlossstats.log 2>&1 &

# Check if the script started successfully
if [ $? -eq 0 ]; then
  echo "Julia script kineticlossstats.jl started successfully."
  echo "Logs are being written to logs/kineticlossstats.log"
else
